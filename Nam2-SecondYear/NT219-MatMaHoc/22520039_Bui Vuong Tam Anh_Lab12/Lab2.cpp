#include <bits/stdc++.h>
#include <iomanip>
#include <sstream>
#include <string>

using namespace std;

#define MAX_STRING 256

uint8_t s_box[256] =
{
	
};


uint8_t mul2[] =
{
	
};


uint8_t mul3[] =
{
	
};


uint8_t inv_s_box[256] =
{
	
};


uint8_t mul9[256] =
{
	
};

uint8_t mul11[256] =
{
	
};

uint8_t mul13[256] =
{
	
};

uint8_t mul14[256] =
{
	
};

const uint8_t rcon[10] = {
    0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};


void KeyExpansion(uint8_t* originalKey,uint8_t roundKeys[10][16]) {
    uint8_t* last4Bytes = originalKey + 12;
    uint8_t* lastRound = originalKey;

    for (int i = 0; i < 10; i++){
        for (int j = 0; j < 16; j += 4){
            if (j == 0) {
                roundKeys[i][0] = last4Bytes[1];
                roundKeys[i][1] = last4Bytes[2];
                roundKeys[i][2] = last4Bytes[3];
                roundKeys[i][3] = last4Bytes[0];

                for (int k = 0; k < 4; k++){
                    roundKeys[i][k] = s_box[roundKeys[i][k]];
                }

                roundKeys[i][0] ^= rcon[i];

                for (int k = 0; k < 4; k++){
                    roundKeys[i][k] ^= lastRound[k];
                }
            }
            else{
                for (int k = 0; k < 4; k++){
                    roundKeys[i][j + k] = lastRound[k] ^ last4Bytes[k];
                }
            }
            if (i == 0 && j==0){
                last4Bytes = roundKeys[0];
            }
            else{
                last4Bytes += 4;
            }
            lastRound += 4;
        }
        if (i == 0) {
            lastRound = roundKeys[i];
        }
    }
}

void SubBytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = s_box[state[i]];
    }
}

void ShiftRows(uint8_t* state) {
    uint8_t tmp[16];

    tmp[0] = state[0];
    tmp[1] = state[5];
    tmp[2] = state[10];
    tmp[3] = state[15];

    tmp[4] = state[4];
    tmp[5] = state[9];
    tmp[6] = state[14];
    tmp[7] = state[3];

    tmp[8] = state[8];
    tmp[9] = state[13];
    tmp[10] = state[2];
    tmp[11] = state[7];

    tmp[12] = state[12];
    tmp[13] = state[1];
    tmp[14] = state[6];
    tmp[15] = state[11];

    for (int i = 0; i < 16; i++) {
        state[i] = tmp[i];
    }
}

void MixColumns(uint8_t* state) {
    uint8_t tmp[16];

    tmp[0] = (uint8_t)(mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3]);
    tmp[1] = (uint8_t)(state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3]);
    tmp[2] = (uint8_t)(state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]]);
    tmp[3] = (uint8_t)(mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]]);

    tmp[4] = (uint8_t)(mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7]);
    tmp[5] = (uint8_t)(state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7]);
    tmp[6] = (uint8_t)(state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]]);
    tmp[7] = (uint8_t)(mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]]);

    tmp[8] = (uint8_t)(mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11]);
    tmp[9] = (uint8_t)(state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11]);
    tmp[10] = (uint8_t)(state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]]);
    tmp[11] = (uint8_t)(mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]]);

    tmp[12] = (uint8_t)(mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15]);
    tmp[13] = (uint8_t)(state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15]);
    tmp[14] = (uint8_t)(state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]]);
    tmp[15] = (uint8_t)(mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]]);

    for (int i = 0; i < 16; i++){
        state[i] = tmp[i];
    }
}

void AddRoundKey(uint8_t* state,uint8_t* roundKey){
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

void InvSubBytes(uint8_t* state){
    for (int i = 0; i < 16; i++){
        state[i] = inv_s_box[state[i]];
    }
}

void InvShiftRows(uint8_t* state) {
    uint8_t tmp[16];

    tmp[0] = state[0];
    tmp[1] = state[13];
    tmp[2] = state[10];
    tmp[3] = state[7];

    tmp[4] = state[4];
    tmp[5] = state[1];
    tmp[6] = state[14];
    tmp[7] = state[11];

    tmp[8] = state[8];
    tmp[9] = state[5];
    tmp[10] = state[2];
    tmp[11] = state[15];

    tmp[12] = state[12];
    tmp[13] = state[9];
    tmp[14] = state[6];
    tmp[15] = state[3];

    // Copy tmp to state
    for (int i = 0; i < 16; i++) {
        state[i] = tmp[i];
    }
}

void InvMixColumns(uint8_t* state) {
    uint8_t tmp[16];

    tmp[0] = (uint8_t)(mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul9[state[3]]);
    tmp[1] = (uint8_t)(mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]]);
    tmp[2] = (uint8_t)(mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]]);
    tmp[3] = (uint8_t)(mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]]);

    tmp[4] = (uint8_t)(mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul9[state[7]]);
    tmp[5] = (uint8_t)(mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]]);
    tmp[6] = (uint8_t)(mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]]);
    tmp[7] = (uint8_t)(mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]]);

    tmp[8] = (uint8_t)(mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul9[state[11]]);
    tmp[9] = (uint8_t)(mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]]);
    tmp[10] = (uint8_t)(mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]]);
    tmp[11] = (uint8_t)(mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]]);

    tmp[12] = (uint8_t)(mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul9[state[15]]);
    tmp[13] = (uint8_t)(mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]]);
    tmp[14] = (uint8_t)(mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]]);
    tmp[15] = (uint8_t)(mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]]);
    for (int i = 0; i < 16; i++) {
        state[i] = tmp[i];
    }
}

void AES_Encrypt(uint8_t* state,uint8_t* key,uint8_t roundKeys[10][16]) {
    AddRoundKey(state,key);

    for (int i = 0; i < 9 ; i++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state,roundKeys[i]);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state,roundKeys[9]);
}

void AES_Decrypt(uint8_t* state,uint8_t* key,uint8_t roundKeys[10][16]) {
    AddRoundKey(state,roundKeys[9]);
    InvShiftRows(state);
    InvSubBytes(state);
    for (int i = 9 - 1; i >= 0 ; i--) {
        AddRoundKey(state,roundKeys[i]);
        InvMixColumns(state);
        InvShiftRows(state);
        InvSubBytes(state);
    }
    AddRoundKey(state,key);
}


void CBC_Encryption(uint8_t* data,int bytes,uint8_t* key,uint8_t* IV) {
    auto roundKeys = new uint8_t[10][16];
    KeyExpansion(key,roundKeys);

    int cbcRounds = bytes / 16;
    uint8_t prevCBCResult[16] = { 0 };

    for (int i = 0; i < cbcRounds; i++){
        if (i == 0) {
            for (int j = 0; j < 16; j++){
                *(data + i * 16 + j) ^= IV[j];
            }
        }
        else {
            for (int j = 0; j < 16; j++) {
                *(data + i * 16 + j) ^= prevCBCResult[j];
            }
        }
        AES_Encrypt(data + i * 16,key,roundKeys);
        for (int j = 0; j < 16; j++) {
            prevCBCResult[j] = *(data + i * 16 + j);
        }
    }
    delete[] roundKeys;
}

void CBC_Decryption(uint8_t* data,int bytes,uint8_t* key,uint8_t* IV) {
    auto roundKeys = new uint8_t[10][16];
    KeyExpansion(key,roundKeys);

    int cbcRounds = bytes / 16;
    uint8_t prevCBCResult[16] = { 0 };
    uint8_t cipherText[16] = {
     0 };

    for (int i = 0; i < cbcRounds; i++) {
        for (int j = 0; j < 16; j++) {
            cipherText[j] = *(data + i * 16 + j);
        }
        AES_Decrypt(data + i * 16,key,roundKeys);

        if (i == 0) {
            for (int j = 0; j < 16; j++) {
                *(data + i * 16 + j) ^= IV[j];
                prevCBCResult[j] = cipherText[j];
            }
        }
        else {
            for (int j = 0; j < 16; j++) {
                *(data + i * 16 + j) ^= prevCBCResult[j];
                prevCBCResult[j] = cipherText[j];
            }
        }
    }
    delete[] roundKeys;
}

int char2int(char input)
{
  if(input >= '0' && input <= '9')
    return input - '0';
  if(input >= 'A' && input <= 'F')
    return input - 'A' + 10;
  if(input >= 'a' && input <= 'f')
    return input - 'a' + 10;
  throw std::invalid_argument("Check your input carefully bro");
}

void hex2bin(const char* src,uint8_t* target)
{
  while(*src && src[1])
  {
    *(target++) = char2int(*src)*16 + char2int(src[1]);
    src += 2;
  }
}
void printHex(uint8_t* buffer,int bufferSize) {
    for (int i = 0; i < bufferSize; i++) {
        wprintf(L"%02x",buffer[i]);
    }
    wcout << '\n';   
}


#ifdef _WIN32
#include <io.h>
#include <fcntl.h>
#else
#endif

#include <locale>
using std::wstring_convert;
#include <codecvt>
using  std::codecvt_utf8;
wstring string_to_wstring (const std::string& str);
string wstring_to_string (const std::wstring& str);

#ifdef _WIN32
#include <io.h>
#include <fcntl.h>
#endif


int main(){
    #ifdef __linux__
    setlocale(LC_ALL,"");
    #elif _WIN32
    _setmode(_fileno(stdin),_O_U16TEXT);
    _setmode(_fileno(stdout),_O_U16TEXT);
    #endif

    wstring wHexKey, wHexIV;// Set key and iv
    wcout << "Please enter your secret key in hex: ";
    ws(wcin);
    getline(wcin,wHexKey);
    wcout << "Please enter your iv in hex: ";
    ws(wcin);
    getline(wcin,wHexIV);
    string HexKey = wstring_to_string(wHexKey);
    string HexIV = wstring_to_string(wHexIV);

    if (HexKey.size() != 32 || HexIV.size() != 32){
        wcout << "Please enter your secret key and iv correctly !\n";
        exit(0);
    }
    char* key_string = &HexKey[0];
    char* iv_string = &HexIV[0];
    uint8_t* key = new uint8_t[16];
    uint8_t* iv = new uint8_t[16];
    hex2bin(key_string,key);
    hex2bin(iv_string,iv);
    while (true) {
        int aescipher;
        wcout << "\nWhat do you want to do\n"
            << "1. Encrypt;\n"
            << "2. Decrypt;\n"
            << "3. exit\n"
            << "Please enter your number: ";
        wcin >> aescipher;
        switch (aescipher){
        case 1: {
            wstring wplaintext;
            wcout << "Hehe give me something you want to hide from plain sight: ";
            ws(wcin);
            getline(wcin,wplaintext);
            string plaintext = wstring_to_string(wplaintext);
            int plaintextSize = plaintext.size();
            while (plaintextSize % 16 != 0) {
                plaintext += "#";
                plaintextSize++;
            }
            uint8_t plaintextBuffer[2048];
            copy(plaintext.begin(),plaintext.end(),plaintextBuffer);
            CBC_Encryption(plaintextBuffer,plaintextSize,key,iv);
            wcout << "Now you can hide it from: ";
            printHex(plaintextBuffer,plaintextSize);
            break;
        }
        case 2:{
            wstring wciphertext;
            wcout << "Type something you want to recover it: ";
            ws(wcin);
            getline(wcin,wciphertext);
            string cipher = wstring_to_string(wciphertext);
            int ciphertextSize = cipher.size() / 2;
            uint8_t* ciphertextBuffer = new uint8_t[ciphertextSize];
            char* cstr = &cipher[0];
            hex2bin(cstr,ciphertextBuffer);
            CBC_Decryption(ciphertextBuffer,ciphertextSize,key,iv);
            ostringstream ss;
            for (int i = 0; i < ciphertextSize; i++){
                ss << ciphertextBuffer[i];
            }
            string recovered = ss.str();
            for (size_t i = 0; i < recovered.size(); i++){
                if (recovered[i] == 35){
                     recovered.erase(recovered.begin() + i,recovered.end()); 
                    break;
                }
            }
            wcout << "Here is your recovered plaintext was: " << string_to_wstring(recovered) << endl;
            break;
        }
        case 3:{
            exit(0);
            break;
        }    
        default: {
            wcout << "Please enter a valid number (1-3)!\n";
            break;
        }
        }
    }
}
wstring string_to_wstring(const string& str){
    wstring_convert<codecvt_utf8<wchar_t>> towstring;
    return towstring.from_bytes(str);
}

string wstring_to_string(const wstring& str){
    wstring_convert<codecvt_utf8<wchar_t>> tostring;
    return tostring.to_bytes(str);
}
